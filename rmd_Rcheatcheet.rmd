...
Load all libraries at beginning of list
...
## Loading and Manipuating packages and data frames

### Data importing and cleaning using Janitor
...
```{r}
mtcars <- mtcars %>%  mutate(sum = select(., disp:drat) %>% apply(1, sum, na.rm=TRUE))
#apply() takes Data frame or matrix as an input and gives output in vector (i.e.many columns to one list)
...
```{r}
#prepare new dataset
mtcars <- mtcars %>% mutate(carnames=rownames(mtcars)) # make the rownames into a 'joinable' column
mtcars_extradata <- mtcars %>% select(cyl)
mtcars_extradata <- mtcars_extradata %>% mutate(carnames=rownames(mtcars_extradata)) # make the rownames into a 'joinable' column
mtcars_extradata <- mtcars_extradata %>% mutate(valves=cyl*4)
mtcars_extradata <- mtcars_extradata %>% select(-cyl)

kable(glimpse(mtcars_extradata)) %>% kable_minimal()
...
```{r}
kable(mtcars %>% group_by(cyl) %>% summarise(Average=mean(hp), StDev=sd(hp))) %>% kable_styling(full_width = FALSE) %>% kable_minimal()
```
...
kable(dat2 %>% summarise(across(x:z,funcs))) %>% kable_styling(full_width = FALSE) %>% kable_minimal()
...
df.sum <- mtcars %>%  select(mpg,cyl,hp) %>% summarise(across(everything(),list(mean=mean,sd=sd)))# make sure brackets are correct

df.sum

df.longer <- df.sum%>% pivot_longer(col=everything(),  names_to = c("Attribute",".value"),
  names_sep = "_"
)
df.longer
```
